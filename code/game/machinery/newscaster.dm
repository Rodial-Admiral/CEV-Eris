...
/obj/machinery/newscaster/attack_ai(mob/living/user)
...
/obj/machinery/newscaster/attack_hand(mob/user)
	...
	var/dat = text("<HEAD><TITLE>Newscaster</TITLE></HEAD><H3>Newscaster Unit #[src.unit_no]</H3>")
	scan_user(user)
	switch(screen)
		if(0)
			...
			dat+= "<BR><BR><A href='?src=\ref[user];mach_close=newscaster_main'>Exit</A>"
			if(securityCaster)
	...
	user << browse(dat, "window=newscaster_main;size=400x600")
